...
export function StackArchitect() {
  ...
  const [openCategories, setOpenCategories] = useState<Set<TechnologyCategory>>(() => new Set(["frontend", "frontendArchitecture", "backend", "backendArchitecture"]));
      ...
      <section className="architect-workbench lab-workbench" aria-label="Architect Lab">
        ...
        <main className="lab-center">
          <header className="lab-commandbar">
            ...
            <div className="lab-global-actions"><button type="button" onClick={() => setContextOpen(true)}>Architect info</button><button type="button" onClick={() => setCommandOpen(true)} aria-label="Open command palette">⌘ K</button><button type="button" onClick={share}>{shareLabel}</button></div>
          </header>
          ...
          <section className={`compatibility-summary compatibility-summary--${analysis.level}`} aria-label="Compatibility status">
            <div className="compatibility-summary__verdict" role="status"><i aria-hidden="true">{analysis.level === "incompatible" ? "×" : "✓"}</i><div><span>Compatibility status</span><strong>{analysis.level === "conditional" ? "Compatible" : levelLabels[analysis.level]}</strong>{analysis.level === "conditional" && <span className="sr-only">Conditional fit</span>}<p>{analysis.summary}</p></div></div>
          ...
          <section className="architecture-title-card lab-architecture-title">
            <i aria-hidden="true">◇</i><div><div className="lab-title-line"><h1><span className="sr-only">Compose a stack: </span>{example.title}</h1><label className="variant-select"><span className="sr-only">Active architecture variant</span><select value={activeVariant.id} onChange={(event) => { setActiveVariantId(event.target.value); setMode("compare"); }}>{variants.map((variant, index) => <option value={variant.id} key={variant.id}>Variant {String.fromCharCode(65 + index)}</option>)}</select></label><button type="button" aria-label="Rename architecture" onClick={renameVariant}>✎</button></div><p>{example.subtitle}</p></div>
            <div className="lab-title-meta"><span>Created</span><strong>May 18, 2025</strong></div><div className="lab-title-meta"><span>Updated</span><strong>2 min ago</strong></div><button type="button" className="lab-save-button" onClick={() => setVariants((current) => current.map((variant) => ({ ...variant, preferred: variant.id === activeVariant.id })))}>▱ Save</button><button type="button" className="lab-more-button" aria-label="More architecture actions">•••</button>
          </section>
          ...
          {mode === "compare" && differences.length > 0 && <DifferenceSummary baseline={baseline.name} active={activeVariant.name} differences={differences} visible={showDifferences} toggle={() => setShowDifferences((current) => !current)} />}

          <section className="topology-card lab-topology">
            <nav className="lab-workspace-tabs" aria-label="Architecture workspace sections"><button type="button" aria-current={inspectorTab === "flow" ? "page" : undefined} onClick={() => setInspectorTab("flow")}>Architecture map</button><button type="button" aria-current={inspectorTab === "decisions" ? "page" : undefined} onClick={() => setInspectorTab("decisions")}>Decisions</button><button type="button" aria-current={inspectorTab === "technologies" ? "page" : undefined} onClick={() => setInspectorTab("technologies")}>Technologies</button><button type="button" aria-current={inspectorTab === "contracts" ? "page" : undefined} onClick={() => setInspectorTab("contracts")}>Contracts</button><button type="button" aria-current={inspectorTab === "risks" ? "page" : undefined} onClick={() => setInspectorTab("risks")}>Risks</button><button type="button" aria-current={inspectorTab === "sources" ? "page" : undefined} onClick={() => setInspectorTab("sources")}>Sources</button><div className="architecture-legend"><span><i className="legend-request" /> Request</span><span><i className="legend-data" /> Data</span><span><i className="legend-async" /> Async</span><span><i className="legend-delivery" /> Control</span></div></nav>
            ...
            <div className="architecture-map" data-testid="architecture-map">
              <ReactFlow key={`${activeVariant.id}-${lens}-${Object.values(activeVariant.selection).join("-")}`} nodes={nodes} edges={edges} nodesConnectable={false} fitView fitViewOptions={{ padding: .08 }} minZoom={.36} maxZoom={1.7} onInit={setFlowInstance} onNodeClick={(_, node) => { const index = example.steps.findIndex((step) => step.nodeIds.includes(node.id)); if (index >= 0) setActiveStepIndex(index); setInspectorTab("flow"); }} onEdgeClick={(_, edge) => { const index = example.steps.findIndex((step) => step.edgeIds.includes(edge.id)); if (index >= 0) setActiveStepIndex(index); setInspectorTab("contracts"); }} proOptions={{ hideAttribution: true }}>
                <Background color="#d7dce5" gap={28} size={1} /><Controls showInteractive={false} position="bottom-left" /><MiniMap pannable zoomable nodeStrokeWidth={3} nodeColor="#dcd7fa" nodeStrokeColor="#8b6cf1" maskColor="rgba(112, 76, 242, .08)" position="bottom-left" />
              </ReactFlow>
              <div className="lab-map-tools" aria-label="Map controls"><button type="button" onClick={() => flowInstance?.fitView({ padding: .08 })}>Fit</button><button type="button" aria-label="Zoom out" onClick={() => flowInstance?.zoomOut()}>−</button><span>{Math.round((flowInstance?.getZoom() ?? 1) * 100)}%</span><button type="button" aria-label="Zoom in" onClick={() => flowInstance?.zoomIn()}>+</button><button type="button" aria-label="Toggle map fullscreen" onClick={() => document.querySelector<HTMLElement>(".architecture-map")?.requestFullscreen()}>⛶</button><button type="button" aria-pressed={lens === "runtime"} onClick={() => { setLens("runtime"); setActiveStepIndex(0); }}>Runtime</button><button type="button" aria-pressed={lens === "delivery"} onClick={() => { setLens("delivery"); setActiveStepIndex(0); }}>Delivery</button></div>
            ...
            <div className="lab-map-metrics"><div><i>▣</i><strong>{example.nodes.filter((node) => node.kind === "service" || node.kind === "frontend").length}</strong><span>Services<small>Deployable units</small></span></div><div><i>⌘</i><strong>{example.edges.length}</strong><span>Dependencies<small>Internal & external</small></span></div><div><i>DB</i><strong>{example.nodes.filter((node) => node.kind === "data").length}</strong><span>Data stores<small>Persistent storage</small></span></div><div><i>▦</i><strong>{example.nodes.filter((node) => node.kind === "actor" || node.kind === "boundary").length}</strong><span>External systems<small>Third-party services</small></span></div><div><i>●</i><strong>{architectureId(activeVariant.selection) === "microservices" ? "High" : "Medium"}</strong><span>Complexity<small>Estimated</small></span></div></div>
          </section>

          <ArchitectureWalkthrough steps={example.steps} activeIndex={activeStepIndex} setActive={setActiveStepIndex} />
...
function TechnologyPalette({ query, setQuery, visibleCategories, openCategories, setOpenCategories, selection, analysis, choose, blockers, reset }: PaletteProps) {
  return <aside className="technology-picker" aria-label="Choose technologies"><div className="technology-picker__heading"><strong>Technology palette</strong></div><label className="technology-search"><span aria-hidden="true">⌕</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search technologies..." aria-label="Search technologies" /><kbd>⌘K</kbd></label><div className="technology-picker__groups">{visibleCategories.map((category) => <details key={category} open={Boolean(query.trim()) || openCategories.has(category)} onToggle={(event) => { if (query.trim()) return; const isOpen = event.currentTarget.open; setOpenCategories((current) => { const next = new Set(current); if (isOpen) next.add(category); else next.delete(category); return next; }); }}><summary><span className="technology-summary__caret" aria-hidden="true">⌄</span><strong>{categoryLabels[category]}</strong><b>{analysis.selected[category].name}</b><span className="technology-summary__check" aria-hidden="true">✓</span></summary><fieldset><legend className="sr-only">{categoryLabels[category]}</legend><div className="technology-options">{technologiesByCategory[category].filter((technology) => !query.trim() || technology.name.toLowerCase().includes(query.toLowerCase()) || categoryLabels[category].toLowerCase().includes(query.toLowerCase())).map((technology) => <label key={technology.id} className={selection[category] === technology.id ? "is-selected" : ""}><input type="radio" name={category} value={technology.id} checked={selection[category] === technology.id} onChange={() => choose(category, technology.id)} /><span className="technology-option__mark" aria-hidden="true">{technology.mark}</span><span><strong>{technology.name}</strong><small>{technology.runtime}</small></span>{selection[category] === technology.id && <span className="technology-option__check" aria-hidden="true">✓</span>}</label>)}</div></fieldset></details>)}</div><div className="technology-picker__actions"><div><strong>{categoryOrder.length} / {categoryOrder.length} selected</strong></div><i aria-hidden="true" /><dl><div><dt>Blocking</dt><dd>{blockers}</dd></div><div><dt>Warnings</dt><dd>{analysis.findings.filter((finding) => finding.level === "warning").length}</dd></div><div><dt>Decisions</dt><dd>{analysis.findings.length}</dd></div></dl><button type="button" onClick={() => document.querySelector(".architecture-map")?.scrollIntoView({ behavior: "smooth", block: "center" })}>✣ Generate architecture</button><button className="technology-picker__reset" type="button" onClick={reset}>Reset selection</button></div></aside>;
}
...
type InspectorProps = { tab: InspectorTab; setTab: (tab: InspectorTab) => void; analysis: ReturnType<typeof analyzeStack>; example: ReturnType<typeof buildArchitectureExample>; activeStep: ReturnType<typeof buildArchitectureExample>["steps"][number]; selectedConnectionId: string | null; setSelectedConnectionId: (id: string) => void; playbooks: ReturnType<typeof selectedGuides>; sources: ReturnType<typeof officialSourcesForAnalysis>; failures: ReturnType<typeof compareFailureModes>["failureModes"]; architectureVariantId: ArchitectureVariantId; openExport: () => void };
function InspectorPanel({ tab, setTab, analysis, example, activeStep, selectedConnectionId, setSelectedConnectionId, playbooks, sources, failures, architectureVariantId, openExport }: InspectorProps) {
  const selectedConnection = analysis.connections.find((connection) => connection.id === selectedConnectionId);
  return <aside className="inspector-panel" aria-label="Architecture inspector"><header className="inspector-toolbar"><button type="button" onClick={() => navigator.clipboard.writeText(window.location.href)}>⌯ Share</button><button type="button" onClick={openExport}>⇧ Export</button><button type="button" aria-label="More inspector actions">•••</button></header><nav role="tablist" aria-label="Inspector sections">{inspectorTabs.map((item) => <button type="button" role="tab" aria-selected={tab === item} onClick={() => setTab(item)} key={item}>{item[0].toUpperCase() + item.slice(1)}{item === "risks" && <span>{failures.length}</span>}</button>)}</nav><div className="inspector-content" tabIndex={0} aria-label={`${tab} inspector content`}>
    {tab === "flow" && <><div className="inspector-heading"><span>REQUEST FLOW</span><strong className="inspector-title">How this architecture works</strong><p>End-to-end request flow through the system.</p></div><ol className="inspector-flow">{example.steps.map((step, index) => <li className={step.id === activeStep.id ? "is-active" : ""} key={step.id}><span>{index + 1}</span><div><strong>{step.title}</strong><p>{step.whatHappens}</p></div></li>)}</ol><section className="inspector-key-decisions"><header><strong>Key decisions</strong><button type="button" onClick={() => setTab("decisions")}>View all</button></header>{analysis.findings.slice(0, 3).map((finding) => <article key={finding.title}><i aria-hidden="true">{finding.level === "warning" ? "!" : "✓"}</i><div><strong>{finding.title}</strong><p>{finding.detail}</p></div><span>{finding.level === "warning" ? "Conditional" : "Good fit"}</span></article>)}</section></>}
  ...
  </div><footer className="inspector-recommendation"><span>{analysis.findings.filter((finding) => finding.level === "warning").length} warnings</span><strong>Review compatibility warnings</strong><button type="button" onClick={() => setTab("decisions")} aria-label="Review compatibility warnings">→</button></footer></aside>;
}
